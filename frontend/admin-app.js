...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentSectionTitleElement = document.getElementById(
  ...
  async function loadMenuManagementContent() {
    ...
    document
      .getElementById("add-menu-item-btn")
    ...
    document
      ...
      .addEventListener("submit", handleMenuItemFormSubmit);
  }
    ...
    try {
      ...
      if (categories.length === 0) {
        categoriesListDiv.innerHTML =
          "<p>Категории не найдены. Добавьте первую!</p>";
        return;
      }
      ...
      table.querySelectorAll(".edit-category-btn").forEach((btn) => {
        btn.addEventListener("click", (e) =>
          openCategoryModal(e.target.dataset.id, e.target.dataset.name)
        );
      });
  ...
  async function handleCategoryFormSubmit(event) {
    ...
    const url = id
      ? `${API_BASE_URL_ADMIN}/categories/${id}`
      : `${API_BASE_URL_ADMIN}/categories`;
  ...
  async function deleteCategory(id) {
    if (
      !confirm(
        "Вы уверены, что хотите удалить эту категорию? Это также удалит все связанные с ней блюда!"
      )
    )
      return;
    const currentAuthToken = localStorage.getItem("authToken");
    ...
    try {
      ...
      if (menuItems.length === 0) {
        menuItemsListDiv.innerHTML =
          "<p>Позиции меню не найдены. Добавьте первую!</p>";
        return;
      }
                    ...
                    ${menuItems
                      .map(
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-warning edit-menu-item-btn" data-id="${
                                  item.id
                                }">Ред.</button>
                                <button class="btn btn-sm btn-danger delete-menu-item-btn" data-id="${
                                  item.id
                                }">Удал.</button>
                            </td>
                        </tr>
      ...
      table.querySelectorAll(".edit-menu-item-btn").forEach((btn) => {
        btn.addEventListener("click", (e) =>
          openMenuItemModal(e.target.dataset.id)
        );
      });
      table.querySelectorAll(".delete-menu-item-btn").forEach((btn) => {
        btn.addEventListener("click", (e) =>
          deleteMenuItem(e.target.dataset.id)
        );
      });
    } catch (error) {
  ...
  async function populateCategoryDropdown() {
    ...
    try {
      const response = await fetch(`${API_BASE_URL_ADMIN}/categories`);
      if (!response.ok)
        throw new Error("Ошибка загрузки категорий для дропдауна");
      const categories = await response.json();
  ...
  async function openMenuItemModal(id = null) {
    ...
    if (id) {
      try {
        ...
        document.getElementById("menu-item-price").value = parseFloat(
          itemData.price
        ).toFixed(2);
        ...
        showAdminMessage(
          `Ошибка загрузки данных для редактирования: ${error.message}`
        );
  ...
  async function handleMenuItemFormSubmit(event) {
    ...
    const menuItemData = {
      ...
      description: document
        .getElementById("menu-item-description")
        .value.trim(),
      ...
      image_url:
        document.getElementById("menu-item-image-url").value.trim() || null,
    };
    ...
    if (
      ...
      showAdminMessage(
        "Заполните все обязательные поля (Название, Цена, Категория) корректно."
      );
    ...
    const method = id ? "PUT" : "POST";
    const url = id
      ? `${API_BASE_URL_ADMIN}/menu/${id}`
      : `${API_BASE_URL_ADMIN}/menu`;
    const currentAuthToken = localStorage.getItem("authToken");
  ...
  async function loadOrdersManagementContent() {
    const ordersManagementSection = document.getElementById(
    ...
    document
      .getElementById("apply-order-filters-btn")
  ...
  async function fetchAndDisplayAdminOrders(filters = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.message || `Ошибка загрузки заказов: ${response.status}`
        );
      }
      ...
      if (orders.length === 0) {
        ordersListDiv.innerHTML =
          "<p>Заказы не найдены (или не соответствуют фильтрам).</p>";
        return;
      }
                    ...
                    ${orders
                      .map(
                        ...
                        <tr>
                            ...
                            <td>${parseFloat(order.total_amount).toFixed(
                              2
                            )} руб.</td>
                            <td>
                                <select class="order-status-select" data-order-id="${
                                  order.id
                                }">
                                    <option value="new" ${
                            ...
                            <td>
                                <button class="btn btn-sm btn-info view-order-details-btn" data-order-id="${
                                  order.id
                                }">Детали</button>
                            </td>
                        </tr>
  ...
  async function updateOrderStatus(orderId, newStatus) {
    ...
    const oldStatus = selectElement
      ? selectElement.dataset.previousStatus
      : null; // Сохраним старый статус

    // Устанавливаем предыдущий статус, чтобы можно было его восстановить
    if (selectElement && !oldStatus) {
      // Ищем выбранную опцию до изменения, чтобы сохранить ее value
      // Это немного сложно, так как событие 'change' уже произошло.
      // Проще всего, если бы мы сохраняли начальный статус при отрисовке.
      // Для простоты, пока будем полагаться на то, что если ошибка, пользователь увидит сообщение.
      // Либо можно передавать старый статус в функцию.
    }
    try {
      const response = await fetch(
        `${API_BASE_URL_ADMIN}/orders/${orderId}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.message || `Ошибка обновления статуса: ${response.status}`
        );
      }
      ...
      if (selectElement) {
        selectElement.dataset.previousStatus = newStatus; // Обновляем "предыдущий" статус на новый
      }
    } catch (error) {
      cconsole.error(`Ошибка при обновлении статуса заказа ${orderId}:`, error);
      showMessage(`Ошибка: ${error.message}`, "error");
      // Возвращаем select к предыдущему значению, если оно было сохранено
      // и если ошибка не связана с тем, что такого статуса нет.
      // Для более надежного отката, лучше перезагрузить заказы или хранить начальное состояние.
      if (selectElement && oldStatus && newStatus !== oldStatus) {
        // selectElement.value = oldStatus; // Это может вызвать рекурсивный вызов, если обработчик не снять
        // Проще всего в случае ошибки просто попросить пользователя обновить или перезагрузить список
        // или не делать ничего, так как пользователь уже видит ошибку.
        // Самый простой вариант - не откатывать UI, а просто показать ошибку.
        // Пользователь может сам выбрать правильный статус снова или обновить страницу.
      } else if (selectElement) {
        // Если не было oldStatus, просто оставляем как есть, но с ошибкой
  ...
  async function openOrderDetailsModal(orderId) {
    ...
    try {
      ...
      let detailsHtml = `
                ...
                <p><strong>Тип:</strong> ${
                  order.order_type === "table" ? "За столиком" : "Самовывоз"
                }</p>
                ${
                  order.order_type === "table"
                    ? `<p><strong>Столик:</strong> ${
                        order.table_number || "N/A"
                      }</p>`
                    : ""
                }
                ${
                  order.order_type === "takeaway"
                    ? `<p><strong>Время самовывоза:</strong> ${
                        order.pickup_time || "N/A"
                      }</p>`
                    : ""
                }
                <p><strong>Статус:</strong> ${order.status}</p>
                <p><strong>Сумма:</strong> ${parseFloat(
                  order.total_amount
                ).toFixed(2)} руб.</p>
      ...
      if (order.items && order.items.length > 0) {
        ...
        order.items.forEach((item) => {
          detailsHtml += `<li>${item.menu_item_name} - ${
            item.quantity
          } шт. x ${parseFloat(item.price_at_order).toFixed(2)} руб.</li>`;
        });
        detailsHtml += "</ul>";
      } else {
  ...
  async function loadTablesManagementContent() {
    const tablesManagementSection = document.getElementById(
      ...
      tables.forEach((table) => {
        ...
        tableCard.innerHTML = `
                    ...
                    <div class="table-card-status">
                        <select class="table-status-select-card" data-table-id="${
                          table.id
                        }">
                            <option value="free" ${
                            ...
                            <option value="reserved" ${
                              table.status === "reserved" ? "selected" : ""
                            }>Забронирован</option>
                        </select>
                    </div>
        ...
        tableCard
          .querySelector(".table-status-select-card")
          .addEventListener("change", (e) => {
            updateTableStatus(
              e.target.dataset.tableId,
              e.target.value,
              tableCard
            );
          });
        ...
        tableCard
          .querySelector(".edit-table-btn")
          .addEventListener("click", () =>
            openTableModal(table.id, table.table_number, table.status)
          );
        tableCard
          .querySelector(".delete-table-btn")
          .addEventListener("click", () => deleteTable(table.id));
      });
  ...
  function openTableModal(id = null, number = "", status = "free") {
    ...
    document.getElementById("table-status-select-modal").value =
      status || "free";
    ...
    document
      .getElementById("table-status-select-modal")
      .closest(".form-group").style.display = id ? "none" : "block";
  ...
  async function handleTableFormSubmit(event) {
    ...
    if (!table_number) {
      showAdminMessage("Номер/название столика не может быть пустым.");
  ...
  async function updateTableStatus(tableId, newStatus, tableCardElement) {
    const authToken = localStorage.getItem("authToken");
    // Сохраняем старый класс, чтобы можно было откатить, если что
    const oldStatusClass = tableCardElement
      ? Array.from(tableCardElement.classList).find((cls) =>
          cls.startsWith("status-")
        )
      : null;
    const oldSelectValue = tableCardElement
      ? tableCardElement.querySelector(".table-status-select-card").value
      : newStatus;
    try {
      const response = await fetch(
        `${API_BASE_URL_ADMIN}/tables/${tableId}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      showMessage("Статус столика обновлен!", "success");

      // Обновляем класс у карточки для изменения цвета
      if (tableCardElement && oldStatusClass) {
        tableCardElement.classList.remove(oldStatusClass);
      ...
      showMessage(`Ошибка: ${error.message}`, "error");
      // Откатываем UI, если возможно
      if (tableCardElement) {
        if (oldStatusClass) {
          // Если старый класс был, возвращаем его
          tableCardElement.classList.remove(
            ...Array.from(tableCardElement.classList).filter((cls) =>
              cls.startsWith("status-")
            )
          ); // Удаляем текущий status-* класс
          tableCardElement.classList.add(oldStatusClass); // Добавляем старый
        }
        // Возвращаем значение селекта
        const selectElement = tableCardElement.querySelector(
          ".table-status-select-card"
        );
        if (selectElement && selectElement.value !== oldSelectValue) {
          selectElement.value = oldSelectValue; // Старый статус, который был до попытки изменения
        }
      }
    }
  }
  ...
  function showAdminMessage(message, type = "success", duration = 5000) {
    ...
    messageContainer.className = `message admin-message ${type}`; // Добавляем класс admin-message для возможных отличий
    messageContainer.textContent = message;

    // Вставляем в main-content, чтобы было видно внутри админ-лейаута
    const mainContent = document.querySelector(".main-content");
    if (mainContent) {
      mainContent.insertBefore(messageContainer, mainContent.firstChild); // Вставляем в начало main-content
    } else {
      document.body.appendChild(messageContainer); // Фоллбэк, если .main-content не найден
    }

    // Анимация и удаление
    // Используем keyframes из admin-style.css
    messageContainer.style.opacity = "0"; // Начальное состояние для анимации
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        // Даем время для fade-out перед удалением
        if (messageContainer.parentNode) {
          messageContainer.remove();
        }
      }, 500); // 0.5s для fade-out
    }, duration - 500); // Общая длительность минус время на fade-out
  }
});
